...
#define EPOLL_EVENTS_MASK1  (EPOLLIN|EPOLLET)
...
void handle_accept(int efd, int fd)
{
    ...
    set_nonblock(infd);
    update_events(efd,infd,EPOLLIN | EPOLLOUT | EPOLLET,EPOLL_CTL_ADD);
}
...
static int sock_init(int port)
{
    ...
    ret = set_socket(fd, SO_REUSEPORT,true);
...
int main(int argc, char *argv[])
{
    ...
    update_events(efd,fd,EPOLL_EVENTS_MASK1 ,EPOLL_CTL_ADD);
    for(;;)
    {
