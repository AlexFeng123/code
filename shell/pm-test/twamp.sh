...
curl -k -X POST -H 'Content-Type:application/json;ext=nn' -H "X-Auth-Token: ${token}" -d "{  \"v4addrs\": [\"172.23.222.226/24\"],\"name\": \"intf1\",\"type\": \"if\" }" https://${target}:443/mit/me/1/twamp/iprouter/1/ipif/1
echo

curl -k -X POST -H 'Content-Type:application/json;ext=nn' -H "X-Auth-Token: ${token}" -d "{ \"id\":\"1\",\"ipif\":\"/me=1/twamp/iprouter=1/ipif=1\", \"usrlbl\":\"cc1\", \"port\": \"/me=1/eqh=shelf,1/eqh=slot,1/eq=card/ptp=port,1/ctp=eth/mac\",\"mode\": \"unauth\",\"ctrlen\": false}" https://${target}:443/mit/me/1/twamp/twpcc/1
ss_list=
...
do
    ...
    date
    ssport=$(expr 1000 + $ss);
    curl -k -X POST -H 'Content-Type:application/json;ext=nn' -H "X-Auth-Token: ${token}" -d "{ \"id\":\"${ss}\", \"ipif\":\"/me=1/twamp/iprouter=1/ipif=1\",\"starttm\":{\"imm\":{}}, \"usrlbl\":\"ss1\", \"numpkts\": 100000,\"ssdscp\": 2,\"ssudpprt\":${ssport} , \"srudpprt\": 22345,\"pktsize\":100,\"rsptimeo\": 5, \"pktintvl\": 100,\"srip\": \"172.23.222.229\",\"pattern\": \"all0\",\"vlanlst\": [ { \"ethtyp\": \"ctag\",\"vlanpos\": \"outer\", \"vlantag\": \"4094-1\"}]}"  https://${target}:443/mit/me/1/twamp/twpss/${ss} 
    echo
    if [ ${ss} -eq 1 ];then
