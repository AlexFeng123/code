...
do
    ...
    do
        ...
        ## create fp
        if [ $vid -eq 1 ];then
        curl -k -X POST -H 'Content-Type:application/json;ext=nn' -H "X-Auth-Token: ${token}" -d '{"ingmbrs":"4094"}' https://${target}:443/mit/me/1/eqh/shelf,1/eqh/slot,1/eq/card/ptp/port,${PORT}/ctp/eth/ctp/fp-${vid}/fp
        else
        ...
        ## create fp
        if [ $vid -eq 1 ];then
        curl -k -X POST -H 'Content-Type:application/json;ext=nn' -H "X-Auth-Token: ${token}" -d '{"ingmbrs":"4094"}' https://${target}:443/mit/me/1/eqh/shelf,1/eqh/slot,1/eq/card/ptp/port,${PORTB}/ctp/eth/ctp/fp-${vid}/fp
        else
...
curl -k -X PATCH -H 'Content-Type:application/json-patch+json;ext=nn' -H "X-Auth-Token: ${token}" -d '[{"op":"replace", "path":"/admin", "value":"is"}]' https://${target}:443/mit/me/1/sn/1/snc/1/sm
