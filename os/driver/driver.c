...
static ssize_t alex_write(struct file *filp, const char *buf, size_t len, loff_t *offset) {
	...
	if(strncmp(buf,start_cmd,strlen(start_cmd)) == 0){
                ts_test_start();
	}

	if(strncmp(buf, stop_cmd, strlen(stop_cmd))  == 0) {
		ts_test_stop();
	}
...
static int alex_probe(struct platform_device *pdev) {
	...
	if(ret < 0 ) {
		...
		unregister_chrdev_region(devno,ALEX_NUMS);
		return ret;
	}
